...
class OnnxAudioFileHelper {
    ...
    static func loadAudioFileWithResampling(url: URL, targetSampleRate: Float = 16000) -> [Float]? {
        ...
        do {
            // 1. Load audio file
            let audioFile = try AVAudioFile(forReading: url)
            OnnxLogHelper.log("time test 02")
            // 2. Set up the audio engine for resampling
            let audioEngine = AVAudioEngine()
            let audioPlayerNode = AVAudioPlayerNode()
            audioEngine.attach(audioPlayerNode)
            OnnxLogHelper.log("time test 022")
            ...
            OnnxLogHelper.log("time test 05")
            try audioEngine.start()
            OnnxLogHelper.log("time test 051")
            ...
            if let channelData = audioBuffer?.floatChannelData {
                let frameLength = Int(audioBuffer?.frameLength ?? 0)
                let bufferPointer = UnsafeBufferPointer(start: channelData[0], count: frameLength)
                let audioData = Array(bufferPointer)
                return audioData
            }
            return nil
